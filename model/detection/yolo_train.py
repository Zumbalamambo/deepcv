...
def main():
    ...
    tf.logging.warn(
        '(width, height)=(%d, %d), (cell_width, cell_height)=(%d, %d)' % (width, height, cell_width, cell_height))
    ...
    with tf.name_scope('batch'):
        image_rgb, labels = utils.data.load_image_labels(paths, len(names), width, height, cell_width, cell_height,
                                                         config)
    ...
    with tf.name_scope('optimizer'):
        try:
            ...
            learning_rate = tf.train.exponential_decay(args.learning_rate, global_step, decay_steps, decay_rate,
                                                       staircase=staircase)
            tf.logging.warn(
                'using a learning rate start from %f with exponential decay (decay_steps=%d, decay_rate=%f, staircase=%d)' % (
    ...
    if args.transfer:
        ...
        def init_fn(sess):
            ...
            tf.logging.warn(
                'transferring from global_step=%d, learning_rate=%f' % sess.run((global_step, learning_rate)))

    elif args.logdir:
        path = os.path.expanduser(os.path.expandvars(args.logdir))
        tf.logging.war('load checkpoint from ' + path)
        model_path = tf.train.latest_checkpoint(path)
        init_assign_op, init_feed_dict = slim.assign_from_checkpoint(model_path, variables_to_restore)
        ...
        def init_fn(sess):
            sess.run(init_assign_op, init_feed_dict)
            tf.logging.warn(
                'loadding from global_step=%d, learning_rate=%f' % sess.run((global_step, learning_rate)))
    else:
    ...
    slim.learning.train(train_op, logdir, master=args.master,
                        train_step_kwargs={'should_log': tf.constant(True, tf.bool)},
                        is_chief=(args.task == 0), log_every_n_steps=100,
...
def make_args():
    ...
    parser.add_argument('--summary_secs', default=60, type=int, help='seconds to save summaries')
    parser.add_argument('--save_secs', default=300, type=int, help='seconds to save model')
