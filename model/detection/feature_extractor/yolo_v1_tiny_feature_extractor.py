...
def tiny(net, classes, boxes_per_cell, training=False):
    ...
    with slim.arg_scope([slim.layers.conv2d], kernel_size=[3, 3], activation_fn=tfnet.leaky_relu), slim.arg_scope(
            [slim.layers.max_pool2d], kernel_size=[2, 2], padding='SAME'):
        index = 0
    ...
    net = slim.layers.fully_connected(net, cell_width * cell_height * (classes + boxes_per_cell * 5),
                                      activation_fn=None, scope='%s/fc' % scope)
