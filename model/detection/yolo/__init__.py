...
class Model(object):
    def __init__(self, net, scope, classes, boxes_per_cell, training=False):
        _, self.cell_height, self.cell_width, _ = tf.get_default_graph().get_tensor_by_name(
            scope + '/conv:0').get_shape().as_list()
...
class Objectives(dict):
    def __init__(self, model, mask, prob, coords, offset_xy_min, offset_xy_max, areas):
        ...
        with tf.name_scope('objectives'):
            ...
            self['coords'] = tf.identity(tf.reduce_sum(tf.expand_dims(mask_best, -1) * coords_dist) / cnt,
                                         name='coords')
